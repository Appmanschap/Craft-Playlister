...
class Playlists extends BaseRelationField
{
    ...
    {
        return Craft::t('youtube-playlist-importer', 'YouTube Playlists ({pluginName})', ['pluginName' => YoutubePlaylistImporter::$plugin?->name]);
    }
    ...
    {
        return Craft::t('youtubeplaylistimporter', 'Add a YouTube playlist');
    }
    ...
    {
        return sprintf('\\%s|\\%s<\\%s>', PlaylistQuery::class, ElementCollection::class, PlaylistElement::class);
    }
    ...
    {
        $condition = PlaylistElement::createCondition();
        $condition->queryParams = ['id', 'playlistId'];
        return $condition;
    }
}
